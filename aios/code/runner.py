...
class CodeRunner:
    ...
    def _resolve_auth_env(self, auth_mode: Optional[str] = None) -> Dict[str, str]:
        ...
        * ``None``           -- inherits the current environment as-is
        ...
        if auth_mode == "api_key":
            # Try to get the key from AIOS config, then fall back to env
            api_key = None
            if self._config and hasattr(self._config, "_api_key"):
                api_key = self._config._api_key
            if not api_key:
                # Check the parent AIOSConfig
    ...
    def launch_interactive(
        ...
        try:
            ...
            return LaunchResult(success=True, return_code=0)
        except Exception as e:
            return LaunchResult(success=False, return_code=-1, error=str(e))

        # Persist session metadata if we can determine the session id.
        # For bare interactive launches we won't have one, but for
        # --resume we do.
        if session_id:
            self._save_session(CodeSession(
                session_id=session_id,
                working_directory=cwd,
                prompt_summary=(prompt or "")[:100],
            ))

        return LaunchResult(success=success, return_code=return_code)
